...
namespace onionreq
{
  ...
  class ChannelEncryption
  {
   public:
    ...
    std::string
    encrypt(EncryptType type, std::string_view plaintext, const x25519_pubkey& pubkey) const;
    std::string
    decrypt(EncryptType type, std::string_view ciphertext, const x25519_pubkey& pubkey) const;

    // AES-CBC encryption.
    std::string
    encrypt_cbc(std::string_view plainText, const x25519_pubkey& pubKey) const;
    std::string
    decrypt_cbc(std::string_view cipherText, const x25519_pubkey& pubKey) const;

    // AES-GCM encryption.
    std::string
    encrypt_gcm(std::string_view plainText, const x25519_pubkey& pubKey) const;
    std::string
    decrypt_gcm(std::string_view cipherText, const x25519_pubkey& pubKey) const;
    ...
    std::string
    encrypt_xchacha20(std::string_view plaintext, const x25519_pubkey& pubKey) const;
    std::string
    decrypt_xchacha20(std::string_view ciphertext, const x25519_pubkey& pubKey) const;
