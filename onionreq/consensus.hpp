...
namespace onionreq
{
  ...
  /// @brief a path selection algorithm that determines how we build paths and gives a way to propagate feedback on errors and success 
  ...
  public:
    ...
    virtual NodeListFetcher_Base *CreateNodeFetcher() const = 0;

    /// @brief construct a new path selection algorithm
    virtual PathSelection_Base *CreatePathSelector() const = 0;
