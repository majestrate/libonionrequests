...
namespace onionreq
{
  ...
  namespace detail
  {
    ...
    void
    load_from_hex(void* buffer, size_t length, std::string_view hex);
    void
    load_from_bytes(void* buffer, size_t length, std::string_view bytes);
    std::string
    to_hex(const unsigned char* buffer, size_t length);
  ...
  struct alignas(size_t) key_base : std::array<unsigned char, KeyLength>
  {
    ...
    hex() const
    {
      return detail::to_hex(this->data(), KeyLength);
    }
    explicit operator bool() const
    {
      return *this != detail::null_bytes<KeyLength>;
    }
    ...
    maybe_from_hex(std::string_view hex)
    {
      try
      {
        return from_hex(hex);
  ...
  inline std::ostream&
  operator<<(std::ostream& o, const legacy_pubkey& pk)
  {
    return o << pk.hex();
  }
  inline std::ostream&
  operator<<(std::ostream& o, const x25519_pubkey& pk)
  {
    return o << pk.hex();
  }
  inline std::ostream&
  operator<<(std::ostream& o, const ed25519_pubkey& pk)
  {
    return o << pk.hex();
  }
  ...
  legacy_pubkey
  parse_legacy_pubkey(std::string_view pubkey_in);
  ed25519_pubkey
  parse_ed25519_pubkey(std::string_view pubkey_in);
  x25519_pubkey
  parse_x25519_pubkey(std::string_view pubkey_in);
...
namespace std
{
  ...
  struct hash<onionreq::pubkey_base<Derived, N>>
  {
    size_t
    operator()(const onionreq::pubkey_base<Derived, N>& pk) const
    {
      // pubkeys are already random enough to use the first bytes directly as a good (and fast) hash
      // value
  ...
  template <>
  struct hash<onionreq::legacy_pubkey> : hash<onionreq::legacy_pubkey::PubKeyBase>
  {};
  template <>
  struct hash<onionreq::x25519_pubkey> : hash<onionreq::x25519_pubkey::PubKeyBase>
  {};
  template <>
  struct hash<onionreq::ed25519_pubkey> : hash<onionreq::ed25519_pubkey::PubKeyBase>
  {};
