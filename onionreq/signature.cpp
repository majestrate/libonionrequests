...
static_assert(crypto_generichash_BYTES == onionreq::HASH_SIZE, "Wrong hash size!");
...
namespace onionreq
{
  ...
  void
  cn_fast_hash(const void*, size_t, char*)
  {
    abort();
  }

  static ec_scalar
  monero_hash_to_scalar(const void* input, size_t size)
  {
  ...
  hash
  hash_data(std::string_view data)
  {
    hash hash;
    crypto_generichash(
        hash.data(),
  ...
  static std::array<unsigned char, S>
  hash_concatenate(const T&... v)
  {
  ...
  signature
  generate_signature(const hash& prefix_hash, const legacy_keypair& keys)
  {
     ...
     *
     * But this relies on a random scalar x, which is undesirable; Ed25519, in contrast, gets x from
     * H(S
  ...
  bool
  check_signature(const signature& sig, const hash& prefix_hash, const legacy_pubkey& pub)
  {
  ...
  signature
  signature::from_base64(std::string_view signature_b64)
  {
