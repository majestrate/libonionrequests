...
// We construct an Ed25519-like signature with one important difference: where Ed25519 calculates `r
// = H(S || M) mod L` (where S is the second half of the SHA-512 hash of the secret key) we instead
// calculate `r = H(a || M || Z) mod L`.
//
// This deviates from Signal's XEd25519 specified derivation of r in that we use a personalized
// Black2b hash (for better performance and cryptographic properties), rather than a custom-prefixed
// SHA-512 hash.
bytes<32> xed25519_compute_r(const bytes<32>& a, ustring_view msg) {
    ...
    crypto_generichash_blake2b_init_salt_personal(&st, nullptr, 0, 64, nullptr, personality.data());
...
void ed25519_hram(unsigned char* S, const unsigned char* R, const bytes<32>& A, ustring_view msg) {
    bytes<64> hram;
...
bool verify(ustring_view signature, ustring_view curve25519_pubkey, ustring_view msg) {
    ...
    return 0 == crypto_sign_ed25519_verify_detached(signature.data(), msg.data(), msg.size(), ed_pubkey.data());
}
