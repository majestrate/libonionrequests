...
namespace onionreq
{
    ...
    load_from_hex(void* buffer, size_t length, std::string_view hex)
    {
      ...
      if (hex.size() != 2 * length)
        throw std::runtime_error{
            "Hex key data is invalid: expected " + std::to_string(length) + " hex digits, received "
  ...
  std::string
  ed25519_pubkey::snode_address() const
  {
  ...
  template <typename T>
  static T
  parse_pubkey(std::string_view pubkey_in)
  {
    T pk{};
