...
namespace onionreq
{
  namespace
  {
    ...
    calculate_shared_secret(const x25519_seckey& seckey, const x25519_pubkey& pubkey)
    {
    ...
    std::array<uint8_t, crypto_scalarmult_BYTES>
    derive_symmetric_key(const x25519_seckey& seckey, const x25519_pubkey& pubkey)
    {
  ...
  EncryptType
  parse_enc_type(std::string_view enc_type)
  {
  ...
  std::string
  ChannelEncryption::encrypt(
      EncryptType type, std::string_view plaintext, const x25519_pubkey& pubkey) const
  {
    switch (type)
    {
      case EncryptType::xchacha20:
        return encrypt_xchacha20(plaintext, pubkey);
      case EncryptType::aes_gcm:
        return encrypt_gcm(plaintext, pubkey);
      case EncryptType::aes_cbc:
        return encrypt_cbc(plaintext, pubkey);
    }
    throw std::runtime_error{"Invalid encryption type"};
  }

  std::string
  ChannelEncryption::decrypt(
      EncryptType type, std::string_view ciphertext, const x25519_pubkey& pubkey) const
  {
    switch (type)
    {
      case EncryptType::xchacha20:
        return decrypt_xchacha20(ciphertext, pubkey);
      case EncryptType::aes_gcm:
        return decrypt_gcm(ciphertext, pubkey);
      case EncryptType::aes_cbc:
        return decrypt_cbc(ciphertext, pubkey);
    }
  ...
  std::string
  ChannelEncryption::encrypt_cbc(std::string_view plaintext_, const x25519_pubkey& pubKey) const
  {
    return encrypt_openssl(
        EVP_aes_256_cbc(), 0, to_uchar(plaintext_), calculate_shared_secret(private_key_, pubKey));
  }

  std::string
  ChannelEncryption::decrypt_cbc(std::string_view ciphertext_, const x25519_pubkey& pubKey) const
  {
    return decrypt_openssl(
        EVP_aes_256_cbc(), 0, to_uchar(ciphertext_), calculate_shared_secret(private_key_, pubKey));
  }

  std::string
  ChannelEncryption::encrypt_gcm(std::string_view plaintext_, const x25519_pubkey& pubKey) const
  {
    return encrypt_openssl(
        EVP_aes_256_gcm(),
        16 /* tag length */,
  ...
  std::string
  ChannelEncryption::decrypt_gcm(std::string_view ciphertext_, const x25519_pubkey& pubKey) const
  {
    return decrypt_openssl(
        EVP_aes_256_gcm(),
        16 /* tag length */,
      ...
      bool local_first)
  {
    ...
    if (0
        != crypto_scalarmult(
            key.data(), local_sec.data(), remote_pub.data()))  // Use key as tmp storage for aB
  ...
  std::string
  ChannelEncryption::encrypt_xchacha20(
  ...
  std::string
  ChannelEncryption::decrypt_xchacha20(
