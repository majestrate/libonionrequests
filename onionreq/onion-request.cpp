...
main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (arg == "--aes-cbc"sv)
    {
      enc_type = EncryptType::aes_cbc;
      continue;
    }
    if (arg == "--random"sv)
    {
      enc_type = std::nullopt;
      continue;
    }
      ...
      [&got, omq_addr](auto, auto err) {
        try
        {
          throw std::runtime_error{
              "Failed to connect to oxend @ " + omq_addr.full_address() + ": " + std::string{err}};
        }
        catch (...)
        {
        ...
        try
        {
          ...
          for (auto& sn : sns)
          {
            ...
            if (e.size() != 64 || x.size() != 64 || !oxenmq::is_hex(x) || !oxenmq::is_hex(e))
              throw std::runtime_error{
                  sn.at("service_node_pubkey").get<std::string>()
                  + " is missing ed/x25519 pubkeys"};
            aux_keys.emplace(
                legacy_pubkey::from_hex(pk),
  ...
  try
  {
    ...
    if (chain.size() != pubkeys.size())
      throw std::runtime_error{"Missing x25519 pubkeys"};
    if (chain.empty())
      throw std::runtime_error{"Need at least one SN pubkey"};
...
random_etype()
{
  ...
  size_t i = dist(rng);
  return i == 0 ? EncryptType::aes_cbc : i == 1 ? EncryptType::aes_gcm : EncryptType::xchacha20;
}
    ...
    std::string_view control)
{
  ...
  blob = encode_size(blob.size()) + blob
      + nlohmann::json{{"ephemeral_key", A.hex()}, {"enc_type", to_string(last_etype)}}.dump();
  ...
  try
  {
    body = d.decrypt(final_etype, body, keys.back().second);
    decrypted = true;
  }
  catch (...)
  {}
  ...
  if (decrypted)
  {
    std::cerr << "Body is " << orig_size << " encrypted bytes, decrypted to " << body.size()
              << " bytes:\n";
  }
  else if (oxenmq::is_base64(body))
  {
    body = oxenmq::from_base64(body);
    std::cerr << "Body was " << orig_size << " base64 bytes; decoded to " << body.size()
              << " bytes";
    try
    {
      body = d.decrypt(final_etype, body, keys.back().second);
  ...
  {
    std::cerr << "Body is " << body.size()
              << " bytes (not base64-encoded, not encrypted [or decryption failed])\n";
  }
  std::cerr << std::flush;
